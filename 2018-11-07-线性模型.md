...
\\
这里的-y_i\beta^T\hat{\mathbb{x}_i}不是直接计算得到的，通过等价替换，证明过程略
$$
...
$$
\frac{\partial l(\beta)}{\partial\beta} = \sum^m_{i=1}\hat{\mathbb{x}_i}(p_1(\hat{\mathbb{x}_i}; \beta) - y_i)，我们可以看到p_1其实就是预测的y值，y_i是实际值
\\
