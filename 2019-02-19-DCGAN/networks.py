...
def load_mnist():
    ...
    y_train = tf.cast(tf.one_hot(y_train, depth=10, axis=-1), tf.float32)
...
def mnist_generator(inputs, is_training=True):
    ...
    with tf.contrib.framework.arg_scope(
        ...
        with tf.contrib.framework.arg_scope([layers.batch_norm],
            ...
            h2 = layers.conv2d_transpose(
                h1, gf_dim * 2, [5, 5], stride=[d_h, d_w])
...
def mnist_discriminator(img, conditioning):
    with tf.contrib.framework.arg_scope(
        ...
        h1 = layers.conv2d(h0, df_dim + num_classes, [5, 5], stride=[d_h, d_w])
