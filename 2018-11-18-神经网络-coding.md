...
从协方差热力图中可以发现
* MedInc与房价之间有正相关的线性关系，且是这几个特征中最强的，所以我们可能会考虑使用`DNNLinearCombinedRegressor`并将MedInc作为线性模型特征列，其他作为神经网络特征列；
...
使用Dataset作为输入训练的结果，可以明确地表示，使用Dataset作为输入可以提升整体训练的速度，上面的时间在1小时左右才训练了2轮，使用Dataset10分钟以内就完成了30轮的训练，而且验证集达到了85.65%的准确率：
