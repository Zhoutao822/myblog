...
# 设置优化器optimizer，损失函数loss，这里使用稀疏交叉熵，以及evaluate时使用的准则metrics
model.compile(
...
为了测试代码，我们先不使用整个数据集，而是从原始数据集中划分出一小部分数据进行测试，在确定代码无误后，我们需要修改这部分代码，因为训练集是完整的train文件夹
...
在train完整训练集上，我们使用自定义model最终得到的分数为
